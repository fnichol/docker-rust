...
```console
docker run --rm -v $(pwd):/src fnichol/rust:1.11.0 cargo build --release
```
...
```console
docker run --rm -v $(pwd):/src fnichol/rust:1.11.0 make
```
...
```console
docker run --rm -v $(pwd):/src fnichol/rust:1.11.0 cargo run
```
...
```console
docker run --rm -v $(pwd):/src --volumes-from cargo-cache fnichol/rust:1.11.0 cargo build
```
