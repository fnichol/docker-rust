...
```console
docker run --rm -v "$PWD":/src fnichol/rust:1.6.0 cargo build --release
```
...
```console
docker run --rm -v "$PWD":/src fnichol/rust:1.6.0 make
```
...
```console
docker run --rm -v "$PWD":/src fnichol/rust:1.6.0 cargo run
```
...
```console
docker run --rm -v "$PWD":/src --volumes-from cargo-cache fnichol/rust:1.6.0 cargo build
```
